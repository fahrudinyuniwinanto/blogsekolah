...
class Prestasi_guru extends CI_Controller
{
    ...
    {   
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'prestasi_guru/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'prestasi_guru/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'prestasi_guru/index.html';
            $config['first_url'] = base_url() . 'prestasi_guru/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Prestasi_guru_model->total_rows($q);
        $prestasi_guru = $this->Prestasi_guru_model->get_limit_data($config['per_page'], $start, $q);

        $this->load->library('pagination');
        $this->pagination->initialize($config);

        $data = array(
            'prestasi_guru_data' => $prestasi_guru,
            'q' => $q,
            'pagination' => $this->pagination->create_links(),
            'total_rows' => $config['total_rows'],
            'start' => $start,
            'content' => 'backend/prestasi_guru/prestasi_guru_list',
        );
    ...
    public function lookup()
    {
        ...
        $data = array(
            ...
            'total_rows' => $config['total_rows'],
            'start' => $start,
            'content' => 'backend/prestasi_guru/prestasi_guru_lookup',
        );
        ob_start();
    ...
    {
        sf_validate('R');
        $row = $this->Prestasi_guru_model->get_by_id($id);
        if ($row) {
        $data = array(
		...
		'add_2' => $row->add_2,
		'note' => $row->note,
	    'content' => 'backend/prestasi_guru/prestasi_guru_read',
	    );
            $this->load->view(layout(), $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('prestasi_guru'));
        }
    }
    ...
    public function create() 
    {
        ...
        $data = array(
        'button' => 'Create',
        'action' => site_url('prestasi_guru/create_action'),
	    ...
	    'note' => set_value('note'),
	    'content' => 'backend/prestasi_guru/prestasi_guru_form',
	);
        $this->load->view(layout(), $data);
    }
    ...
    public function create_action() 
    {
		...
		'note' => $this->input->post('note',TRUE),
            ...
            $this->Prestasi_guru_model->insert($data);
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
            'button' => 'Update',
            'action' => site_url('prestasi_guru/update_action'),
    ...
    public function update_action() 
    {
		...
		'note' => $this->input->post('note',TRUE),
            ...
            $this->Prestasi_guru_model->update($this->input->post('id_prestasi', TRUE), $data);
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            ...
            $this->Prestasi_guru_model->delete($id);
	...
	foreach ($this->Prestasi_guru_model->get_all() as $data) {
            $kolombody = 0;
...
/* End of file Prestasi_guru.php */
/* Location: ./application/controllers/Prestasi_guru.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2019-09-29 06:50:19 */
/* http://harviacode.com */
