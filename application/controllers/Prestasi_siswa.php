...
class Prestasi_siswa extends CI_Controller {
	...
	public function index() {
		...
		if ($q != '') {
			$config['base_url'] = base_url() . 'prestasi_siswa/index.html?q=' . urlencode($q);
			$config['first_url'] = base_url() . 'prestasi_siswa/index.html?q=' . urlencode($q);
		} else {
			$config['base_url'] = base_url() . 'prestasi_siswa/index.html';
			$config['first_url'] = base_url() . 'prestasi_siswa/index.html';
		}

		$config['per_page'] = 10;
		$config['page_query_string'] = TRUE;
		$config['total_rows'] = $this->Prestasi_siswa_model->total_rows($q);
		$prestasi_siswa = $this->Prestasi_siswa_model->get_limit_data($config['per_page'], $start, $q);

		$this->load->library('pagination');
		$this->pagination->initialize($config);

		$data = array(
			'prestasi_siswa_data' => $prestasi_siswa,
			'q' => $q,
			'pagination' => $this->pagination->create_links(),
			'total_rows' => $config['total_rows'],
			'start' => $start,
			'content' => 'backend/prestasi_siswa/prestasi_siswa_list',
		);
	...
	public function lookup() {
		...
		$data = array(
			...
			'total_rows' => $config['total_rows'],
			'start' => $start,
			'content' => 'backend/prestasi_siswa/prestasi_siswa_lookup',
		);
		ob_start();
	...
	public function read($id) {
		sf_validate('R');
		$row = $this->Prestasi_siswa_model->get_by_id($id);
		if ($row) {
			$data = array(
				...
				'add_2' => $row->add_2,
				'note' => $row->note,
				'content' => 'backend/prestasi_siswa/prestasi_siswa_read',
			);
			$this->load->view(layout(), $data);
		} else {
			$this->session->set_flashdata('message', 'Record Not Found');
			redirect(site_url('prestasi_siswa'));
		}
	}
	...
	public function create() {
		...
		$data = array(
			'button' => 'Create',
			'action' => site_url('prestasi_siswa/create_action'),
			'id_prestasi' => set_value('id_prestasi'),
			'jenis_prestasi' => set_value('jenis_prestasi'),
			'nama_siswa' => set_value('nama_siswa'),
			'nm_nama_siswa' => set_value('nm_nama_siswa'),
			...
			'note' => set_value('note'),
			'content' => 'backend/prestasi_siswa/prestasi_siswa_form',
		);
		$this->load->view(layout(), $data);
	}
	...
	public function create_action() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->Prestasi_siswa_model->insert($data);
	...
	public function update($id) {
		...
		if ($row) {
			$data = array(
				'button' => 'Update',
				'action' => site_url('prestasi_siswa/update_action'),
				...
				'nm_nama_siswa' => set_value('nm_nama_siswa', $row->nama_siswa),
	...
	public function update_action() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->Prestasi_siswa_model->update($this->input->post('id_prestasi', TRUE), $data);
	...
	public function excel() {
		...
		$namaFile = "prestasi_siswa.xls";
		$judul = "prestasi_siswa";
		$tablehead = 0;
		$tablebody = 1;
		$nourut = 1;
		//penulisan header
		...
		foreach ($this->Prestasi_siswa_model->get_all() as $data) {
			$kolombody = 0;
...
/* Location: ./application/controllers/Prestasi_siswa.php */
